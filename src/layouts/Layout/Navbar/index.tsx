...
export default function Navbar() {
  ...
  const logoSrc =
    theme === "special" ? "/CSC-SemFundo-Special.png" : "/CSC-SemFundo.png";

  const NavItems = ({ inSheet = false }) => (
    <div className="flex items-center space-x-4">
      <Link to="/matches">
        <InteractiveHoverButton
          className={`${inSheet ? "w-full justify-start" : ""}`}
          text="Jogos"
          icon={Calendar}
        />
      </Link>
  ...
  return (
    <nav
      className={`fixed w-full z-30 transition-all duration-300 ease-linear ${
